...
function Carousel({ images }: { images: string[] }) {
  ...
  return (
    <div className="relative h-56 sm:h-72 rounded-xl overflow-hidden mb-5" style={{ background: '#000' }}>
      <img src={fallback[cur] || cur} onError={onError} alt="Bilde av boligen" className="w-full h-full object-cover" />
      {n > 1 && (
        <>
  ...
  return (
    <div className="min-h-screen text-slate-900" style={{ background: '#f7f8fa' }}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        {/* ── Risk results (dashboard layout) ── */}
        {risk && (
          <div className="flex flex-col gap-5">
            {/* Kort oppsummert — full bredde */}
        ...
        {metric && (
          <div className="mt-6 flex flex-col gap-3">
            <button onClick={() => setShowCashflow(v => !v)}
              className="w-full py-3.5 rounded-xl font-bold text-white flex items-center justify-center gap-2 transition-transform hover:scale-[1.01]"
